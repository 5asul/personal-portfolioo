...
export function Projects() {
  return (
    <section id="projects" className="py-16 min-h-screen  relative flex flex-col justify-center bg-gradient-to-r from-blue-500 to-indigo-200  items-center  bg-white text-gray-800 text-center">
      ...
      <div className="flex flex-col h-4 lg:flex-row md:bottom-[300px] lg:flex-wrap xxs:mx-5 absolute lg:top-96  xxs:bottom-80 justify-center items-center lg:gap-8 gap-4">
        {projects.map((project, index) => (
          <div
        key={index}
        className="w-full sm:w-2/3 lg:w-1/3 h-64 xxs:cursor-none xxs:px-3 xxs:py-2 xxs:text-sm  p-6 rounded-xl shadow-2xl hover:shadow-lg transition flex flex-col justify-between"
        
          >
        <div>
          <h3 className="text-xl sm:text-2xl md:text-xl lg:text-3xl xxs:text-base font-semibold mb-2 text-blue-50">{project.title}</h3>
          <p className="text-base sm:text-lg md:text-sm lg:text-xl xxs:text-xs mb-4 text-blue-600">{project.description}</p>
        </div>
        <a
          href={project.link}
          className="inline-block xxs:text-xs lg:text-2xl bg-blue-500 xxs:cursor-none xxs:px-3 xxs:py-2 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 transition"
          target="_blank"
